...
<div class="container mx-auto px-4">
    ...
    <a href="{{ route('songs.index') }}" class="bg-blue-500 hover:bg-blue-700 text-white font-bold py-2 px-4 rounded mb-4 inline-block">Go  to Songs</a>
    @auth
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                @foreach($band as $bands)
                    <div class="bg-white shadow-md rounded-lg p-4">
                        <h2 class="text-xl font-semibold mb-2">{{ $bands->name }}</h2>
                        <p class="text-gray-600 mb-2">Genre: {{ $bands->genre }}</p>
                        <p class="text-gray-600 mb-2">Founded in: {{ $bands->founded }}</p>
                        <a href="{{ route('bands.show', $bands->id) }}" class="text-blue-500 hover:underline">View Details</a>
                    </div>
                @endforeach
            </div>
</div>
